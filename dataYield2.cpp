...
void dataYield2(Int_t run=3206, Double_t ngcCut=2., Double_t betaMin =0.5, Double_t betaMax=1.5, 
Double_t deltaMin=-10., Double_t deltaMax=22., Double_t minEdep=0.7, Double_t curCut=5.)

{

  ofstream outFile;
  outFile.open("dataYield2.txt",ios::app | ios::out );
  ...
  Double_t scale = (Double_t)1/trackEff*psFact;
  ...
  if (gSystem->AccessPathName(froot)==0)
    {
      ...
      tr->SetBranchAddress("P.cal.etottracknorm", &etracknorm);
      ...
      for (Int_t iEvent = 0; iEvent < nEvents; iEvent++) 
	{
	  ...
	  if(ngc > ngcCut && delta > deltaMin && delta < deltaMax && etracknorm > minEdep){
	    if(abs(thd)<0.1 && abs(phd)<0.05 && abs(yd) < 10.0){
		if(curr>curCut)
      ...
      f->Close();
      delete f;
      outFile << run <<"\t"<< goode<<"\t"<<goode_corr/charge << "\t" << charge << "\t" << readReport(run,"BCM4C cut current")<<endl;
