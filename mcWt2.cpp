...
// * 
void mcWt2(string tgt="h",string angle="21", string mom="5p7", string spec="hms"){
  ...
  Double_t target, ebeam, hsec, thetac, tgtDenLen, tgtMass, offset, offseti;
  Double_t charge=0;
  ofstream oFile;

  ofstream outFile;
  outFile.open(Form("pass30_mcWt_%s.txt",spec.c_str()),ios::app | ios::out );
...
//match data cuts
  ...
  Double_t *grz=gr->GetZ();

  Double_t *gr2x=gr2->GetX();
  Double_t *gr2y=gr2->GetY();
  Double_t *gr2z=gr2->GetZ();

  Double_t *gr3x=gr3->GetX();
  Double_t *gr3y=gr3->GetY();
  Double_t *gr3z=gr3->GetZ();
  ...
  //  fmc = "mc/march2021/hms10cm/"+spec+"_"+kin+".root";
  ...
  TString fOut=Form("mcWtOut/pass30/%s_mcWt%s.root",spec.c_str(),kin.c_str());
  ...
  //  sigmac = getCxsec2(grx,gry,grz,thetacrad*180./TMath::Pi(),w2);// born
  sigmac = getCxsec(gr,thetacrad*180./TMath::Pi(),w2);// born
  ...
  for (Int_t i=0; i<nEvents; i++)
	...
	hstheta = acos(cos(thetacrad - yptarrec)*cos(xptarrec));
      ...
      //      born = getCxsec2(grx,gry,grz,thetaini*180./TMath::Pi(),w2);// born
      //      rci = getCxsec2(gr2x,gr2y,gr2z,thetaini*180./TMath::Pi(),w2);// born/rad
      //      rad = getCxsec2(gr3x,gr3y,gr3z,thetaini*180./TMath::Pi(),w2);// rad

      born = getCxsec(gr,thetaini*180./TMath::Pi(),w2);// born
      rci = getCxsec(gr2,thetaini*180./TMath::Pi(),w2);// born/rad
      rad = getCxsec(gr3,thetaini*180./TMath::Pi(),w2);// rad
  ...
  for (Int_t i=0;i<32;i++){
      ...
      //      born = getCxsec2(grx,gry,grz,thetacrad*180./TMath::Pi(),w2);// born
      born = getCxsec(gr,thetacrad*180./TMath::Pi(),w2);// born
