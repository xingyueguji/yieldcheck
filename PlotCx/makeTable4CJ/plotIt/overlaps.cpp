...
void  overlaps(){
  ...
  for(int i=0;i<size;i++){
    for(int j=0;j<size;j++){
      if(w2_v.at(i)==w2_v.at(j)){
	  double match=w2_v.at(i);
	  //did I already find this
	  int sz=perDf.size();
	  bool isThere=0;
	  for(int k=0;k<sz;k++){
	    if(match==perDf.at(k))isThere=true;
	  }
	  if(!isThere){
	    perDf.push_back(match);
	    cout << "Match at w2= " << match <<"\ttheta="<<thetac_v.at(i)<<endl;
	  }
      }
    }
  }
...
/*
      double diff=(xsec_v.at(i)-xsec_v.at(i-1))/xsec_v.at(i);
    if(w2_v.at(i)==w2_v.at(i)
