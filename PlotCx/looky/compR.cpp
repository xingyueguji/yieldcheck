...
void compD(){
  ...
  int c5=0;
  int c6=0;
  FILE *fp1 = fopen("pass308new/xsec_R_Table.txt","r");  
  FILE *fp2 = fopen("pass251new/xsec_R_Table.txt","r");  
  ...
  for (int i=0;i<size;i++){
    ...
    if(ep_v.at(i)!=ep2_v.at(i)&&i!=size-1){

      cout << "erasing   " <<i<< endl;
      cout <<ep_v.at(i)<<"\t"<<ep2_v.at(i)<<"\t"<<i<<endl;
      int j=1;

      while(ep_v.at(i)!=ep2_v.at(i)){
	ep2_v.erase(ep2_v.begin()+i);
	xsec2_v.erase(xsec2_v.begin()+i);	
	if(ep_v.at(i)==ep2_v.at(i))break;
      }
      /*
      if(ep_v.at(i)!=ep2_v.at(i+1)){
	ep2_v.erase(ep2_v.begin()+i);
	cout << "2nd option" << endl;
      }
      
      if(ep_v.at(i+1)==ep2_v.at(i)){
	ep_v.erase(ep_v.begin()+i);
	cout << "2nd option" << endl;
      }
      */
      cout << "Done erasing   " <<i<< endl; 
  ...
  g->SetMarkerStyle(33);
    ...
    g->GetYaxis()->SetRangeUser(-10,10.0);
