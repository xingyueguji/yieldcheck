...
void plot_cx(string target="r", string angle="39", string spec="shms", string pass="pass105", float xbmin=0.0, float xbmax=1.0, float cxmin=0.0, float cxmax = 50.0, float ratiomin = 0.7, float ratiomax =1.3, string xaxis = "xb" ){
 ...
 string psets[5];
 float psetfs[5];
 getMomList(angle,psets,psetfs,"shms");
...
if(target=="r"){
 ...
 // TCanvas *c1=new TCanvas();
  ...
  gm->Draw();
  //  if(target=="r" && spec=="shms" && xaxis=="xb")
{gm_k->Draw("same");gm_cj->Draw("same");gm_k1->Draw("same");}
  ...
  TGraphErrors *grcx6=extractCS("shms",target,angle,psets[0],1,"pass115",xaxis);
  TGraphErrors *grcx7=extractCS("shms",target,angle,psets[1],1,"pass115",xaxis);
  TGraphErrors *grcx8=extractCS("shms",target,angle,psets[2],1,"pass115",xaxis);
  TGraphErrors *grcx9=extractCS("shms",target,angle,psets[3],1,"pass115",xaxis);
 ...
 grcx6->SetMarkerStyle(24);
 grcx7->SetMarkerStyle(24);
 grcx8->SetMarkerStyle(24);
 grcx9->SetMarkerStyle(24);
 ...
 grcx6->SetMarkerSize(.6);
 grcx7->SetMarkerSize(.6);
 grcx8->SetMarkerSize(.6);
 grcx9->SetMarkerSize(.6);
 ...
 grcx6->SetMarkerColor(kCyan+3);
 grcx7->SetMarkerColor(kCyan+3);
 grcx8->SetMarkerColor(kCyan+3);
 grcx9->SetMarkerColor(kCyan+3);
 ...
 if(xaxis=="xb")
   {
     ...
     leg->AddEntry(grcx6,"SHMS Hall C, E12-10-002","p");
