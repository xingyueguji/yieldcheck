...
void runDataYields(char const *angle="39", char const *target="h", char const *pcent="1p3", char const *spec="shms"){
  ...
  while(1){
	...
	//		if(strncmp(spec,"shms",3)==0)dataYield(run,2.,.5,1.5,-10.,22.,0.7,5.,fname);
	//		if(strncmp(spec,"hms",3)==0)dataYield(run,1.5,.5,1.5,-6.,9.,0.7,5.,fname);
      }
    }
  }
  return;
