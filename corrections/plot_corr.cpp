...
#include "flatCorr.cpp"
#include "makeRatio.cpp"

void plot_corr(string angle="21", string spec="shms",float xbmin=0.0, float xbmax=1.0, float ymin=0.93, float ymax = 1.01, int choice =0){
 ...
 c1->Divide(3,1);
 c1->cd(1);
 TGraphErrors *grh1=flatCorr("shms","h",angle,pset[0],choice);
 TGraphErrors *grh2=flatCorr("shms","h",angle,pset[1],choice);
 TGraphErrors *grh3=flatCorr("shms","h",angle,pset[2],choice);
 TGraphErrors *grh4=flatCorr("shms","h",angle,pset[3],choice);

 string prefix;
 if(choice==0)prefix="Tracking Eff.";
 if(choice==1)prefix="Boiling Corr.";
 ...
 TGraphErrors *grd1=flatCorr("shms","d",angle,pset[0],choice);
 TGraphErrors *grd2=flatCorr("shms","d",angle,pset[1],choice);
 TGraphErrors *grd3=flatCorr("shms","d",angle,pset[2],choice);
 TGraphErrors *grd4=flatCorr("shms","d",angle,pset[3],choice);
 ...
 grr1->GetYaxis()->SetRangeUser(ymin,ymax);
  grr1->GetYaxis()->SetRangeUser(0,2.);
