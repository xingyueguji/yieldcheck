...
void runDataYields(char const *angle="21", char const *target="h", char const *pcent="3p3", char const *spec="shms", string scaleDummy="h"){
  ...
  while(1){
	...
	string fname=Form("%s_%s%sdeg%s_dataYield%d.root",spec,tgt,angle,mom,run);
	if(strncmp(tgt,"al",2)==0)fname=Form("%s_%s%sdeg%s_%s_dataYield%d.root",spec,tgt,angle,mom,scaleDummy.c_str(),run);
	cout << run <<"\t"<< fname << endl; 
	if(strncmp(spec,"shms",3)==0)dataYield(run,2.,.5,1.5,-10.,22.,0.7,5.,scaleDummy,fname);
	if(strncmp(spec,"hms",3)==0&&run!=1603)dataYield(run,1.5,.5,1.5,-6.,9.,0.7,5.,scaleDummy,fname);
      }
    }
  }
  return;
