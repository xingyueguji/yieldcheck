...
if(!fdum->IsOpen())return;
 ...
 if (DRAW==1)
   {
     ...
     //     pt->SetFillColor(20);
     c1->cd(4);pt->Draw("BR");
     c1->SaveAs(Form("ratiosOut/pass325/%s_ratios%s.pdf",spec.c_str(),kin.c_str()));
     /*
     //    Figure for write up
 ...
 TFile *oFile=new TFile(Form("ratiosOut/pass325/%s_ratios%s.root",spec.c_str(),kin.c_str()),"RECREATE");
